...
public class DimensionTable implements Serializable {
    ...
    public  void buildCreateTableSql() {
        ...
        colsBuf.append(" `").append(primary_key.getCol_name()).append("` ").append(primary_key.getCol_type()).append(end);

        String delColName = del.getCol_name();
        colsBuf.append(" `").append(delColName).append("` ").append(del.getCol_type()).append(end);
        keyindexBuf.append(", \n KEY `").append(delColName).append("` (`").append(delColName).append("`)");
        ...
        for(Column col : jcolList){
            ...
            keyindexBuf.append(", \n KEY `").append(colName).append("` (`").append(colName).append("`)");
        }
        ...
        String create_table_sql = "CREATE TABLE IF NOT EXISTS  " + table_name + " \n" +
                "(" + colsBuf.toString() +
                "PRIMARY KEY (`" + primary_key.getCol_name() +"`)" +
    ...
    public  void buildCreateTableFlinkSql(String url,String username,String password) {
        ...
        colsBuf.append(" `").append(primary_key.getCol_name()).append("` ").append(getFlinkType(primary_key)).append(end);
        ...
        String create_table_flink_sql = "CREATE TEMPORARY  TABLE " + table_name + " \n" +
                "(" + colsBuf.toString() +
                "PRIMARY KEY (`" + primary_key.getCol_name() +"`) NOT ENFORCED " +
    ...
    public  void buildMainKeySql(Map<String, Set<String>> tablesJoinFieldMap, Map<String, List<String>> joinTableFieldMap,String mainTableKey) {
        ...
        List<String> mainKeyIndexFields = DimensionTableHelper.buildMainKeyIndexFields(mainTableKey, joinTableFieldMap);

        buildJoinPath(myTableFieldList, joinPath, mainKeyIndexFields, joinTableFieldMap, tablesJoinFieldMap);
        ...
        for(int i = 0; i < joinPath.size(); i++){
            ...
            if(newNum == 1){
                String sql = "select " + field + " from "+ tabName + " where " + tabName + POINT + this.getPrimary_key().getCol_name() + " in (?)";
    ...
    private  List<String> geTableFieldList(Map<String, Set<String>> tablesFieldMap, String tableName) {
    ...
    private  void buildJoinPath(List<String>  tableFieldList, List<String> joinPath,List<String> mainKeyIndexFields,Map<String,List<String>> joinTableFieldMap,Map<String, Set<String>> tablesFieldMap){
        for(String tableField: tableFieldList){
            ...
            for(String jointableField: joinTableFieldList){
                ...
                buildJoinPath(myTableFieldList, joinPath, mainKeyIndexFields, joinTableFieldMap,tablesFieldMap);
            }
        }


    }
    public List<String> findMainKeyBySql(Connection sql_connection, List<Map<String, String>> datas) throws Exception {

        return DimensionTableHelper.findMainKeyBySql(sql_connection, datas, main_key_sql, primary_key);
    }
